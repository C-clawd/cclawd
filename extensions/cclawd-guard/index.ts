  ...
  register(api: OpenClawPluginApi) {
    ...
    async function initPersonalDashboard(coreUrl: string): Promise<void> {
      ...
      try {
        ...
        // Check if dashboard is already running (e.g., manually started with pnpm dev)
        // If so, try to connect to it
        const { DASHBOARD_PORT } = await import("./dashboard-launcher.js");
        const isRunning = await checkDashboardRunning(DASHBOARD_PORT);
        if (isRunning) {
    ...
    async function checkDashboardRunning(port: number): Promise<boolean> {
      try {
